...
pub fn try_extract_json_values_from_str(
    text: &str,
) -> Result<Vec<serde_json::Value>, serde_json::Error> {
    let mut out = Vec::new();
    for s in extract_data_lines_from_str(text) {
        out.push(serde_json::from_str::<serde_json::Value>(s)?);
    }
    Ok(out)
}

pub fn try_extract_typed_from_str<T: DeserializeOwned>(
    text: &str,
) -> Result<Vec<T>, serde_json::Error> {
    let mut out = Vec::new();
    for s in extract_data_lines_from_str(text) {
        out.push(serde_json::from_str::<T>(s)?);
    }
    Ok(out)
}
