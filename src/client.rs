...
use crate::types::responses::{ResponsesRequest, ResponsesResponse, ResponseStreamEvent};
use crate::types::images::{ImageGenerationRequest, ImageGenerationResponse};
use crate::types::files::{FileListResponse, FileObject, FileDeleteResponse};
...
impl OpenAI {
    pub fn base_url(&self) -> String {self.base_url.as_str().to_string()}
    ...
    pub fn with_http_client<S: Into<String>>(http: HttpClient, api_key: S) -> Result<Self, Error> {
        Self::builder().http_client(http).api_key(api_key.into()).build()
    }

    pub fn from_env() -> Result<Self, Error> {
        let api_key = std::env::var("OPENAI_API_KEY").map_err(|_| Error::MissingApiKey)?;
        let mut b = Self::builder().api_key(api_key);
        if let Ok(o) = std::env::var("OPENAI_ORG_ID") { b = b.org(o); }
        if let Ok(p) = std::env::var("OPENAI_PROJECT_ID") { b = b.project(p); }
        if let Ok(u) = std::env::var("OPENAI_BASE_URL") { b = b.base_url(u); }
        b.build()
    }

    pub fn builder() -> OpenAIBuilder { OpenAIBuilder::default() }
    ...
    pub async fn chat_completion_stream(&self, mut req: ChatCompletionRequest) -> Result<BoxStream<'static, Result<ChatCompletionChunk, Error>>, Error> {
    ...
    pub async fn responses_stream(&self, mut req: ResponsesRequest) -> Result<BoxStream<'static, Result<ResponseStreamEvent, Error>>, Error> {
        req.stream = Some(true);
    ...
    pub async fn files_upload_bytes(&self, filename: &str, bytes: Vec<u8>, purpose: &str) -> Result<FileObject, Error> {
        ...
        let mut req = self.http.post(url)
            ...
            .multipart(form);
        if let Some(org) = &self.org { req = req.header("OpenAI-Organization", org); }
        if let Some(project) = &self.project { req = req.header("OpenAI-Project", project); }
        let resp = self.execute_with_retry(|| req.try_clone().expect("req clone"), false).await?;
        let status = resp.status();
        if status.is_success() { Ok(resp.json::<FileObject>().await?) } else { Self::map_api_error(status, resp).await }
    }

    pub async fn files_download(&self, file_id: &str) -> Result<Vec<u8>, Error> {
        let mk = || {
            let url = self.base_url.join(&format!("/v1/files/{}/content", file_id)).expect("valid path");
        ...
        let status = resp.status();
        if status.is_success() { Ok(resp.bytes().await?.to_vec()) } else { Self::map_api_error(status, resp).await }
    }

    pub async fn files_delete(&self, file_id: &str) -> Result<FileDeleteResponse, Error> {
        let mk = || {
            let url = self.base_url.join(&format!("/v1/files/{}", file_id)).expect("valid path");
        ...
        let resp = self.execute_with_retry(mk, false).await?;
        let status = resp.status();
        if status.is_success() { Ok(resp.json::<FileDeleteResponse>().await?) } else { Self::map_api_error(status, resp).await }
    }
    ...
    pub async fn responses_stream_text(&self, req: ResponsesRequest) -> Result<String, Error> {
        ...
        while let Some(ev) = stream.next().await{
            ...
            if let Some(text) = ev.clone().output_text.as_deref() {
                out.push_str(text);
            } else if let Some(d) = ev.delta.as_ref().and_then(|v| v.get("output_text")).and_then(|v| v.as_str()) {
                out.push_str(d);
            }
        }
    ...
    async fn post_json<TReq: serde::Serialize, TResp: DeserializeOwned>(&self, path: &str, body: &TReq) -> Result<TResp, Error> {
        let mk = || {
    ...
    async fn get_json<TResp: DeserializeOwned>(&self, path: &str) -> Result<TResp, Error> {
        ...
        let resp = self.execute_with_retry(mk, false).await?;
        let status = resp.status();
        if status.is_success() { Ok(resp.json::<TResp>().await?) } else { Self::map_api_error(status, resp).await }
    }

    async fn post_sse<TReq: serde::Serialize, TEvent: DeserializeOwned + Send + 'static>(&self, path: &str, body: &TReq) -> Result<BoxStream<'static, Result<TEvent, Error>>, Error> {
        let mk = || {
            let url = self.base_url.join(path).expect("valid path");
            let mut req = self.http.post(url)
                .header(header::AUTHORIZATION, format!("Bearer {}", self.api_key))
    ...
    fn sse_json_stream<T: DeserializeOwned + Send + 'static>(resp: reqwest::Response) -> BoxStream<'static, Result<T, Error>> {
    ...
    async fn map_api_error<TResp>(status: StatusCode, resp: reqwest::Response) -> Result<TResp, Error> {
        ...
        if let Ok(env) = serde_json::from_str::<ApiErrorEnvelope>(&text) {
            ...
            Err(Error::UnexpectedStatus { status: status.as_u16(), body: text })
        }
    }
}
...
impl OpenAI {
            ...
            return match res {
                Ok(resp) => {
                    ...
                    if self.should_retry_status(status) && attempt < self.max_retries {
                        let delay = self.retry_delay(attempt, resp.headers().get(header::RETRY_AFTER));
                        attempt += 1;
    ...
    fn retry_delay(&self, attempt: u32, retry_after: Option<&header::HeaderValue>) -> std::time::Duration {
...
impl OpenAIBuilder {
    pub fn api_key(mut self, key: String) -> Self { self.api_key = Some(key); self }
    pub fn base_url<S: Into<String>>(mut self, url: S) -> Self { self.base_url = Some(url.into()); self }
    pub fn org<S: Into<String>>(mut self, org: S) -> Self { self.org = Some(org.into()); self }
    pub fn project<S: Into<String>>(mut self, project: S) -> Self { self.project = Some(project.into()); self }
    pub fn timeout(mut self, timeout: Duration) -> Self { self.timeout = Some(timeout); self }
    pub fn user_agent<S: Into<String>>(mut self, ua: S) -> Self { self.user_agent = Some(ua.into()); self }
    pub fn max_retries(mut self, n: u32) -> Self { self.max_retries = Some(n); self }
    pub fn retry_base_delay(mut self, dur: Duration) -> Self { self.retry_base_delay_ms = Some(dur.as_millis() as u64); self }
    pub fn http_client(mut self, client: HttpClient) -> Self { self.http = Some(client); self }
    pub fn proxy<S: Into<String>>(mut self, url: S) -> Self { self.proxy = Some(url.into()); self }
    ...
    pub fn build(self) -> Result<OpenAI, Error> {
        ...
        let http = if let Some(custom) = self.http {
            ...
            if let Some(t) = self.timeout { http = http.timeout(t); }
            if let Some(ua) = self.user_agent {
                http = http.user_agent(ua);
            } else {
                http = http.user_agent(format!("openai-sdk-rs/{} (+https://crates.io/crates/openai-sdk)", env!("CARGO_PKG_VERSION")));
            }
